...
public class OAuth2RestAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(OAuth2RestAuthenticationFilter.class);
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException, IOException, ServletException {
        ...
        final var oauth2AccessToken = new OAuth2AccessToken(OAuth2AccessToken.TokenType.BEARER, signinRequestDto.getAccessToken().getTokenValue(), Instant.now(), Instant.MAX, signinRequestDto.getAccessToken().getScopes());
        final var oauth2RefreshToken = new OAuth2RefreshToken(signinRequestDto.getRefreshToken().getTokenValue(), Instant.now(), Instant.MAX);
        ...
        final var oauth2RestAuthenticationToken = (OAuth2RestAuthenticationToken) this.getAuthenticationManager().authenticate(new OAuth2RestAuthenticationToken(clientRegistration, oauth2AccessToken, oauth2RefreshToken, additionalParameters));
        final var oauth2AuthenticationToken = new OAuth2AuthenticationToken(oauth2RestAuthenticationToken.getPrincipal(), oauth2RestAuthenticationToken.getAuthorities(), oauth2RestAuthenticationToken.getClientRegistration().getRegistrationId());
        oauth2AuthenticationToken.setDetails(authenticationDetails);
        final var oauth2AuthorizedClient = new OAuth2AuthorizedClient(oauth2RestAuthenticationToken.getClientRegistration(), oauth2AuthenticationToken.getName(), oauth2RestAuthenticationToken.getAccessToken(), oauth2RestAuthenticationToken.getRefreshToken());
