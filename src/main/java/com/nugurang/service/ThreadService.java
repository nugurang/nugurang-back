...
public class ThreadService {
    ...
    public ThreadEntity createThread(ThreadInputDto threadInputDto, Long board) throws NotFoundException {
        ...
        ThreadEntity threadEntity = threadDao.save(ThreadEntity.builder().name(threadInputDto.getName()).board(boardService.getBoard(board)).xrefUserTeam(threadInputDto.getTeam().map(teamId -> xrefUserTeamDao.findByUserIdAndTeamId(userEntity.getId(), teamId).get()).orElse(null)).event(threadInputDto.getEvent().map(eventId -> eventDao.getById(eventId)).orElse(null)).user(userEntity).build());
    ...
    public Optional<ThreadEntity> getThread(Long id) {
        ...
        voteService.createVote(VoteInputDto.builder().article(articleEntity.getId()).voteType(voteTypeDao.findByName("VIEW").get().getId()).build());
        return threadEntity;
    }
