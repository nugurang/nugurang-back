...
public class NotificationService {
    ...
    public NotificationEntity createProjectInvitationNotification(UserEntity userEntity, ProjectInvitationEntity projectInvitationEntity) {
        return createNotification(userEntity, NotificationTypeName.PROJECT_INVITATION, List.of(projectInvitationEntity.getId().toString()));
    }

    public NotificationEntity createTeamInvitationNotification(UserEntity userEntity, TeamInvitationEntity teamInvitationEntity) {
        return createNotification(userEntity, NotificationTypeName.TEAM_INVITATION, List.of(teamInvitationEntity.getId().toString()));
    }

    public NotificationEntity createMatchSuccessNotification(UserEntity userEntity, MatchTypeEntity matchTypeEntity, EventEntity eventEntity, TeamEntity teamEntity) {
        log.info("Match " + eventEntity.getId() + " successfully created matchmaking.");
        return createNotification(userEntity, NotificationTypeName.MATCH_SUCCESS, List.of(matchTypeEntity.getId().toString(), eventEntity.getId().toString(), teamEntity.getId().toString()));
    }

    public NotificationEntity createMatchFailureNotification(UserEntity userEntity, MatchTypeEntity matchTypeEntity, EventEntity eventEntity) {
        log.info("Match " + eventEntity.getId() + " failed to create matchmaking.");
        return createNotification(userEntity, NotificationTypeName.MATCH_FAILURE, List.of(matchTypeEntity.getId().toString(), eventEntity.getId().toString()));
    }
    ...
    public NotificationService(final NotificationDao notificationDao, final NotificationDataDao notificationDataDao, final NotificationTypeDao notificationTypeDao) {
