...
public class WorkService {
    ...
    public WorkEntity createWork(WorkInputDto workInputDto, Long project) {
        return workDao.save(WorkEntity.builder().name(workInputDto.getName()).opened(true).order(workInputDto.getOrder().orElseGet(() -> workDao.findFirstByOrderByOrderDesc().map(prevWorkEntity -> prevWorkEntity.getOrder() + 1).orElse(0))).project(projectDao.findById(project).get()).build());
    }
    ...
    public WorkEntity updateWork(WorkInputDto workInputDto, Long workId) {
        return workDao.save(workDao.findById(workId).map(workEntity -> {
            ...
            workEntity.setOrder(workInputDto.getOrder().orElseGet(() -> workDao.findFirstByOrderByOrderDesc().map(prevWorkEntity -> prevWorkEntity.getOrder() + 1).orElse(0)));
            return workEntity;
        }).get());
    }
