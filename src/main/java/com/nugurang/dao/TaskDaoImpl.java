...
public class TaskDaoImpl implements TaskDaoCustom {
    ...
    public List<TaskEntity> findAllByProjectId(Long project) {
        return queryFactory.selectFrom(taskEntity).innerJoin(workEntity).on(taskEntity.work.id.eq(workEntity.id)).innerJoin(projectEntity).on(workEntity.project.id.eq(projectEntity.id)).where(projectEntity.id.eq(project)).fetch();
    }

    public List<TaskEntity> findAllByUserId(Long user) {
        return queryFactory.selectFrom(taskEntity).innerJoin(xrefUserTaskEntity).on(taskEntity.id.eq(xrefUserTaskEntity.task.id)).where(xrefUserTaskEntity.user.id.eq(user)).fetch();
    }

    //<editor-fold defaultstate="collapsed" desc="delombok">
    @SuppressWarnings("all")
    
    public TaskDaoImpl(final JPAQueryFactory queryFactory) {
