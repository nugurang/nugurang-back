...
public class TeamResolver implements GraphQLResolver<TeamDto> {
    ...
    public List<ProjectDto> projects(TeamDto teamDto) {
        return projectDao.findAllByTeamId(teamDto.getId()).stream().map(entity -> entity.toDto()).collect(Collectors.toList());
    }
    ...
    public TeamResolver(final RoleDao roleDao, final ProjectDao projectDao, final UserDao userDao) {
