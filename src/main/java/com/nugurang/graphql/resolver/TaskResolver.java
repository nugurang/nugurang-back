...
public class TaskResolver implements GraphQLResolver<TaskDto> {
    ...
    public WorkDto work(TaskDto taskDto) {
        return taskDao.findById(taskDto.getId()).get().getWork().toDto();
    }

    public ProgressDto progress(TaskDto taskDto) {
        return taskDao.findById(taskDto.getId()).get().getProgress().toDto();
    }

    public List<PositionDto> positions(TaskDto taskDto) {
        return positionDao.findAllByTaskId(taskDto.getId()).stream().map(entity -> entity.toDto()).collect(Collectors.toList());
    }

    public List<UserDto> users(TaskDto taskDto) {
        return userDao.findAllByTaskId(taskDto.getId()).stream().map(entity -> entity.toDto()).collect(Collectors.toList());
    }

    //<editor-fold defaultstate="collapsed" desc="delombok">
    @SuppressWarnings("all")
    
    public TaskResolver(final WorkDao workerDao, final ProgressDao progressDao, final PositionDao positionDao, final TaskDao taskDao, final UserDao userDao) {
