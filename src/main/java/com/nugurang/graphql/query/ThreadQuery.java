...
public class ThreadQuery implements GraphQLQueryResolver {
    ...
    List<ThreadDto> getThreadsByBoards(List<Long> boards, Integer page, Integer pageSize) {
        return threadService.getThreadsByBoards(boards, page, pageSize).stream().map(entity -> entity.toDto()).collect(Collectors.toList());
    }

    List<ThreadDto> getThreadsByBoardNames(List<String> boards, Integer page, Integer pageSize) {
        return threadService.getThreadsByBoardNames(boards, page, pageSize).stream().map(entity -> entity.toDto()).collect(Collectors.toList());
    }

    List<ThreadDto> getHotThreadsByBoardNames(List<String> boards, Integer page, Integer pageSize) {
        return threadService.getHotThreadsByBoardNames(boards, page, pageSize).stream().map(entity -> entity.toDto()).collect(Collectors.toList());
    }

    //<editor-fold defaultstate="collapsed" desc="delombok">
    @SuppressWarnings("all")
    
    public ThreadQuery(final ThreadService threadService) {
